...
function sendReminders() {
  const assignedActions = pendingActions.filter(a => a.assignee)
  const uniqueAssignees = [...new Set(assignedActions.map(a => a.assignee))]

  if (uniqueAssignees.length === 0) {
    notificationStore.show('No assignees to notify')
    return
  }
